...
# Versión para números NO consecutivos (más general)
def generar_problema_88_16():
    intentos = 0
    max_intentos = 1000
    
    while intentos < max_intentos:
        # Generar diferencia aleatoria
        diferencia = random.randint(10, 200)
        
        # Generar diferencia entre números (no necesariamente 1)
        salto = random.randint(1, 5)  # Puede ser 1, 2, 3, 4, 5
        
        # Para números que difieren en 'salto': (x+salto)² - x² = diferencia
        # (x² + 2*salto*x + salto²) - x² = diferencia
        # 2*salto*x + salto² = diferencia
        # x = (diferencia - salto²) / (2*salto)
