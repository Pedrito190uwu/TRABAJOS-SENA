...
def generar_problema_88_9():
    ...
    while intentos < max_intentos:
        # Generar valores aleatorios para monedas
        valor_moneda1 = random.choice([5, 10, 25, 50])  # centavos
        valor_moneda2 = random.choice([1, 5, 10, 25])   # centavos (diferente)
        
        if valor_moneda1 == valor_moneda2:
            valor_moneda2 = random.choice([1, 5, 10, 25])
        
        # Asegurar que valor_moneda1 > valor_moneda2
        if valor_moneda1 < valor_moneda2:
            valor_moneda1, valor_moneda2 = valor_moneda2, valor_moneda1
        
        # Generar total de monedas
        total_monedas = random.randint(10, 50)
        
        # Generar monto total en centavos
        total_centavos = random.randint(100, 1000)  # $1.00 a $10.00
        
        # La ecuación es: v1*x + v2*(total_monedas - x) = total_centavos
        # (v1 - v2)*x + v2*total_monedas = total_centavos
        # x = (total_centavos - v2*total_monedas) / (v1 - v2)
        ...
        if denominador != 0 and numerador % denominador == 0:
            ...
            if x > 0 and x < total_monedas:
                ...
                print("Ejercicio 88-9 (Adaptación)")
                print("=" * 80)
                print("Enunciado:")
