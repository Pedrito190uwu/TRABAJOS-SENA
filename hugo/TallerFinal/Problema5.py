...
def generar_problema_88_5():
    ...
    while intentos < max_intentos:
        ...
        # Generar excedente aleatorio
        excede = random.randint(10, 200)
        
        # La ecuación es: 2*(total - x) = 3x + excede
        # 2*total - 2x = 3x + excede
        # 2*total - excede = 5x
        # x = (2*total - excede) / 5
        ...
        if numerador > 0 and numerador % 5 == 0:
            ...
            if x > 0 and x < total:
                ...
                print("Ejercicio 88-5 (Adaptación)")
                print("=" * 80)
                print("Enunciado:")
                ...
                print(f"Doble del mayor: 2 × {mayor} = {2*mayor}")
                print(f"Triple del menor: 3 × {x} = {3*x}")
