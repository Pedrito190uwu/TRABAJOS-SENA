...
def generar_problema_88_19():
    ...
    while intentos < max_intentos:
        ...
        factor = random.randint(2, 3)  # Puede ser doble o triple cada día
        resta_viernes = random.randint(10, 100)
        suma_sabado = random.randint(5, 50)
        
        # La ecuación es: x + factor*x + factor²*x + factor³*x + (factor³*x - resta_viernes) + (factor³*x - resta_viernes + suma_sabado) = total_ganado
        # Simplificando: x(1 + factor + factor² + factor³ + factor³ + factor³) - resta_viernes - resta_viernes + suma_sabado = total_ganado
        # x(1 + factor + factor² + 3*factor³) - 2*resta_viernes + suma_sabado = total_ganado
        
        # Pero mejor resolvemos paso a paso:
        # x + f*x + f²*x + f³*x + (f³*x - r) + (f³*x - r + s) = T
        # x(1 + f + f² + f³ + f³ + f³) - r - r + s = T
        # x(1 + f + f² + 3f³) - 2r + s = T
        # x = (T + 2r - s) / (1 + f + f² + 3f³)
        ...
        if suma_coeficientes != 0 and numerador % suma_coeficientes == 0:
            ...
            # Calcular ganancias de cada día
            lunes = x
            martes = factor * x
